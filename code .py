...
#-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#移動平均
def moving_average(span, years):
    filtered_data = data[data['年'].astype(int) >= years]

    filtered_data['始値_移動平均'] = filtered_data['始値'].rolling(window=span, min_periods=1).mean()
    filtered_data['終値_移動平均'] = filtered_data['終値'].rolling(window=span, min_periods=1).mean()
    filtered_data['高値_移動平均'] = filtered_data['高値'].rolling(window=span, min_periods=1).mean()
    filtered_data['安値_移動平均'] = filtered_data['安値'].rolling(window=span, min_periods=1).mean()
    
    return filtered_data

span = 5
years = 2012
moving_data = moving_average(span, years)
print(moving_data)
